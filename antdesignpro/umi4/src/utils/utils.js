...
const storage = {
    set(key, value) {
        localStorage.setItem(key, JSON.stringify({
            time: +new Date(), // 存储了当前的日期
            value
        }));
    },
    get(key, cycle = 2592000000) { //cycle当前token的有效期
        cycle = +cycle;
        if (isNaN(cycle)) cycle = 2592000000;
        let data = localStorage.getItem(key);
        if (!data) return null;
        let { time, value } = JSON.parse(data);
        if ((+new Date() - time) > cycle) { // 大于有效期，移除token值
            storage.remove(key);
            return null;
        }
        return value;
...
const formatTime = function formatTime(time, template) {
    if (typeof time !== "string") {
        // 转化为当前时间‘2022/12/31 06:07:00’ --> 以24小时制显示
    ...
    return template.replace(/\{(\d+)\}/g, (_, $1) => { // 全局匹配template进行年月日替换
        let item = arr[$1] || "00";
